...
void SckBase::setup() {
	...
	//SCK aquapioneers
	timerSet(ACTION_ALERT_POWER_SHORTAGE, 5000, true);
	...
	switch(msgIn.com) {
		case ESP_GET_STATUS_COM: {
			...
			if (comparison == 0) {
				...
				sckOut(F("Failed to add network!!! make sure that you have used the"));
				sckOut(F("format set wifi \"name\" \"password\"  "));
			}
			break;
...
void SckBase::sckIn(String strIn) {
	...
	switch(executedCommand) {
		...
		// Mode commands
			...
			if (requestedMode == MODE_COUNT) {
				sckOut(F("Unrecognized mode, please try again!"));
				sckOut(F("Make sure that the mode used is listed below"));
				sckOut(F(" \"ap\"  \"network\" \n \"shell\" \"esp flash\""));
				sckOut(F(" \"esp bridge\"  \"error\" \n \"first boot\"  \"off\""));
				sckOut(F(" \"sdcard\""));
			}
			break;
		...
		// SD card
			...
			sckOut(String F("chan1: ") + getChann1() + F(" mV"), PRIO_HIGH);
			break;

		} case EXTCOM_GET_CHARGER: {
			sckOut(String F("charger: ") + getCharger() + F(" mV"), PRIO_HIGH);
			break;

		}case EXTCOM_GET_BATTVOLT: {
			sckOut(String F("charger: ") + getBatteryVoltage() + F(" mV"), PRIO_HIGH);
			break;
